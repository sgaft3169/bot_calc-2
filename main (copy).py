...
async def send_subscription_prompt(update: Update) -> None:
    keyboard = [[
        InlineKeyboardButton("📺 Подписаться на канал",
                ...
                [
                    InlineKeyboardButton("✅ Я подписался, проверить",
...
async def subscription_callback(update: Update,
...
async def action_callback(update: Update,
                          context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()

    if query.data == "calc_another":
        await query.edit_message_text(
            "🔢 Новый расчет!\n\nВведите текущий тариф (₽/мес):")
        return CUR
        ...
        callback_update = Update(update_id=update.update_id,
                                 message=query.message,
                                 callback_query=query)
        await history_callback(callback_update, context)
        return ConversationHandler.END
    elif query.data == "finish":
        await query.edit_message_text(
            "👋 Спасибо за использование калькулятора тарифов!\n\nДля нового расчета используйте команду /start или /calculate"
        )
        return ConversationHandler.END
...
async def cur_tariff(update: Update,
                     context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        return CUR
    await update.message.reply_text(
        "Введите а/плату для нового тарифа (₽/мес):")
    return NEW


async def new_tariff(update: Update,
                     context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
...
async def cost(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        return COST
    await update.message.reply_text(
        "За какой период рассчитываем выгоду (лет)?")
    return PERIOD
...
async def generate_and_send_reports(cur,
                                    new,
                                    cost,
                                    period_years,
                                    user_name,
    ...
    for m in range(1, months + 1):
        ...
        rows.append(
            [m, round(cumulative_old),
             round(cumulative_new),
             round(diff)])
    ...
    if not os.path.exists(HISTORY_FILE):
        ...
        ws_hist.append([
            "Дата", "Пользователь", "Текущий тариф", "Новый тариф",
            "Подключение", "Период", "Окупаемость (мес)", "Экономия"
        ])
    else:
        wb_hist = load_workbook(HISTORY_FILE)
        ws_hist = wb_hist.active
    ws_hist.append([
        now, user_name, cur, new, cost, months,
    ...
    if admin_id:
        ...
        await bot.send_document(chat_id=admin_id,
                                document=pdf_buffer,
                                filename="user_выгода.pdf")
        await bot.send_document(chat_id=admin_id,
                                document=excel_buffer,
                                filename="user_выгода.xlsx")
...
async def period(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await generate_and_send_reports(cur=context.user_data['cur'],
    ...
    # Предлагаем рассчитать еще один тариф
    keyboard = [[
        InlineKeyboardButton("🔄 Рассчитать другой тариф",
                             callback_data="calc_another")
    ],
                [
                    InlineKeyboardButton("📊 Показать историю",
...
async def history_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_document(chat_id=update.effective_chat.id,
                                    document=buf,
                                    filename="моя_история.xlsx")


async def calculate_command(update: Update,
                            context: ContextTypes.DEFAULT_TYPE) -> int:
    """Команда /calculate - быстрый доступ к калькулятору тарифов"""
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "🧮 /calculate - Быстрый расчет тарифа\n"
        "📊 /history - Показать вашу историю расчетов\n"
        "❌ /cancel - Отменить текущий ввод\n"
        "❓ /help - Показать это сообщение\n\n"
        "💡 **Подсказка:** После завершения расчета можете сразу начать новый!")
        ...
        states={
            CUR: [
                ...
                CallbackQueryHandler(
                    action_callback,
                    pattern="calc_another|show_history|finish")
            ],
